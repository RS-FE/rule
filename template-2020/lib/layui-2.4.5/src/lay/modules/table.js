...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function(exports){
  ...
  Class.prototype.pullData = function(curr){
    ...
    ,sort = function(){//update by hhb@risencn.com
      if(typeof sortThis === 'object'){
        that.sort(sortThis.field, sortThis.type);
      }
    }
    ,sortThis;
    ...
    if(options.url){ //Ajax请求
      ...
      params[request.startName] = options.limit*(curr-1); //update by hhb@risencn.com

      //记录临时排序信息
      //that.sortKey 是在sort方法内定义和赋值的。
      if(typeof that.sortKey ==='object'){
          sortThis = {};
          sortThis.field = that.sortKey.field;
          sortThis.type = that.sortKey.sort;
      }else  if(typeof options.initSort === 'object'){
          sortThis=options.initSort
      }else{
          sortThis={};
      }
      //构建后台排序信息
      if(!options.autoSort && typeof sortThis === 'object' && sortThis.field !=undefined && sortThis.field !=null && sortThis.field !=""){
          //构建翻页参数
          if(sortThis.type==null || sortThis.type===undefined || sortThis.type===""){
              sortThis.type = "asc";
          }
          var of = {
              colName:sortThis.field
              ,OP:sortThis.type
          };
          var aryOf = [];aryOf[0] = of;
          var newcq = sortThis.type==null?{}:{OF:aryOf};
          //取回原来的where内的strMap.CQ.如果是string，要转换成object
          var oldcq = options.where["strMap.CQ"];
          if(oldcq!=undefined && typeof oldcq === "string"){
              oldcq = JSON.parse(oldcq);
          }
          //覆盖翻页信息
          var newcq = $.extend({},oldcq,newcq);
          options.where["strMap.CQ"] =JSON.stringify(newcq);
      }
      //参数
      ...
      $.ajax({
        ...
        ,success: function(res){
          ...
          if(res[response.statusName] != response.statusCode){
            ...
            //update by hhb@risencn.com
            if(res[response.countName]>0 && res[response.dataName].length==0 && curr!=1){  //说明当前分页请求数据为空
                return that.pullData(curr-1,that.loading());
            }
  ...
  Class.prototype.eachCols = function(callback){
    ...
    //add by hhb@risencn.com
    Class.prototype.dicFieldValue = function(item, field) {
  ...
  Class.prototype.renderData = function(res, curr, count, sort){
    ...
    ,render = function(){ //后续性能提升的重点
      ...
      layui.each(data, function(i1, item1){
        ...
        that.eachCols(function(i3, item3){
          ...
          ,content = that.dicFieldValue(item1, field);
    ...
    //显示隐藏分页栏
    // update by hhb@risencn.com 如果数据为空，显示分页栏
    ...
    //update by hhb@risencn.com 换了顺序
    if(data.length === 0){
        that.renderForm();
  ...
  Class.prototype.sort = function(th, type, pull, formEvent){
    ...
    //update by hhb@risencn.com
    var arr = ['asc','desc'];
    if (!type || !(~jQuery.inArray(type, arr))) {
      delete that.sortKey;
    }
    ...
    if(options.autoSort){
      ...
      // 如果想要默认设置了服务器分页的话点击排序自动去查询的，注释掉上面的那句把下面这块的注释打开即可
       if (formEvent) {
         that.page = options.page ? options.page.curr = 1 : 1;
  ...
  Class.prototype.events = function(){
    ...
    th.on('click', function(e){
      ...
      if(nowType === 'asc'){
        ...
        //type = null;
        // update by hhb@risencn.com
        // 如果默认initSort是倒序，而且点击的拍序列刚好是默认拍序列，会导致无法排序。
          //他这样的设计主要是做其他列的点击功能。如果列当前是倒序，那么再点击，type置为null就是无序，那么就会采用initSort的排序。
          //更新后，只有desc和asc。不存在无序
          type="asc";
      } else {
        type = 'asc';
      }
